...
const whiteListedDomain = ["incog.nebula.christmas"];
async function licenseCheck(req: { headers: { host: any } }, pass: string) {
  try {
...
const plugin = (fastify: any, opts: any, done: any) => {
  fastify.addHook("onRequest", function (req: any, reply: any, next: any) {
    if (
